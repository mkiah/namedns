...
def main():
    ...
    required_args.add_argument('-d', '--domain', dest='domain', type=str, help='The domain to take action on.')
    ...
    namedotcom = namedns.Session(args.username, args.api_token, args.dev) #Forced to use DEV environment while testing
    
    pprint.pprint(namedotcom.list_domains())

    # pprint.pprint(namedotcom.list_records(args.domain))
